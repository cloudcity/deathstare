module Deathstare
  class HerokuApp
    ...
    def self.user_authorized_for_app? user
      HerokuApiV3.get(user: user, url: "/apps/#{Deathstare.config.heroku_app_id}")
      true
    ...
    def self.scale_sidekiq_workers(user, requested)
      HerokuApiV3.patch(
        user: user,
        url: "/apps/#{Deathstare.config.heroku_app_id}/formation/sidekiq",
    ...
    def self.get_number_running_sidekiq_workers(user)
      ...
      dynos = HerokuApiV3.get(
        user: user,
        url: "/apps/#{Deathstare.config.heroku_app_id}/dynos"
      )
