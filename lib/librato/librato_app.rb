...
class LibratoApp
  ...
  def self.create_or_update_instruments(*suite_names)
    suite_names.flatten!
    suite_names = Deathstare::Suite.suites.map(&:to_s) if suite_names.blank?
    suites = suite_names.reduce({}) { |h, s| h[s] = Deathstare::Suite.const_get(s).test_names.map { |name| name.gsub(/\W+/, '_').downcase }; h }
